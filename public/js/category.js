    ...
    $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')} });
       ...
       $('#add').click(function(){  
          ...
          // var committeeunit_id=$('#committeeunit_id').val();
           function fetch(){
              $.ajax({
               type:'GET',
               url:'/admin/category_fetch/'+course_id,
               datType:'json',
              ...
              if(confirm("Are you sure you want to delete this?"))
                   {
                     $.ajax({
                     type:'DELETE',
                     url:'/admin/category_delete/'+delete_id,
                     success:function(response){    
          ...
          $.ajax({
               type:'POST',
               url:'/admin/category_update/'+edit_id,
               data:editData,
               contentType: false,
               processData:false,
               beforeSend : function()
             ...
             $(document).on('click', '.edit_id', function(e){ 
              ...
              $.ajax({
               type:'GET',
               url:'/admin/category_edit/'+edit_id,
               success:function(response){
                  ...
                  if(response.status == 404){
                    ...
                    $('#edit_category_status').val(response.edit_value.category_status);
                    $('#edit_category_name').val(response.edit_value.category_name);
                    $('#edit_id').val(edit_id);
                  }
               }
               });
       ...
       $(document).on('submit', '#add_form', function(e){ 
          ...
          function fetch_data(page, sort_type = "", sort_by = "", search = "") {
            $.ajax({
                url: "/admin/category/fetch_data/" + course_id + "?page=" + page + "&sortby=" + sort_by + "&sorttype=" + sort_type + "&search=" + search,
