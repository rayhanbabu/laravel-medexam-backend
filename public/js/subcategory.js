    ...
    $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')} });
         ...
         $(".js-example-disabled-results").select2();
          ...
          // var committeeunit_id=$('#committeeunit_id').val();
           function fetch(){
              $.ajax({
               type:'GET',
               url:'/admin/sub_category_fetch/'+course_id+'/'+category_id,
               datType:'json',
              ...
              if(confirm("Are you sure you want to delete this?"))
                   {
                     $.ajax({
                     type:'DELETE',
                     url:'/admin/sub_category_delete/'+delete_id,
                     success:function(response){    
          ...
          $.ajax({
               type:'POST',
               url:'/admin/sub_category_update/'+edit_id,
               data:editData,
               contentType: false,
               processData:false,
               beforeSend : function()
             ...
             $(document).on('click', '.edit_id', function(e){ 
              ...
              $.ajax({
               type:'GET',
               url:'/admin/sub_category_edit/'+edit_id,
               success:function(response){
                  ...
                  if(response.status == 404){
                    ...
                    $('#edit_sub_category_status').val(response.edit_value.sub_category_status);
                    $('#edit_sub_category_name').val(response.edit_value.sub_category_name);
                    $('#edit_id').val(edit_id);
                  }
               }
               });
       ...
       $(document).on('submit', '#add_form', function(e){ 
          ...
          function fetch_data(page, sort_type = "", sort_by = "", search = "") {
            $.ajax({
                url: "/admin/sub_category/fetch_data/" + course_id + "/"+category_id+"?page=" + page + "&sortby=" + sort_by + "&sorttype=" + sort_type + "&search=" + search,
