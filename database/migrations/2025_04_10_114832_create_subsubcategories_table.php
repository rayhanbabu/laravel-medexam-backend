...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('subsubcategories', function (Blueprint $table) {
            ...
            $table->string('sub_sub_category_name')->comment('Name of the category');
            $table->integer('serial')->nullable()->comment('Optional serial number for course ordering');
            
            $table->foreignId('created_by')->constrained('users')->onDelete('cascade');
            $table->foreignId('updated_by')->nullable()->constrained('users')->onDelete('set null');

            $table->integer('sub_sub_category_status')->comment('0 = Inactive, 1 = Active'); // Adjust based on your enum/status logic
